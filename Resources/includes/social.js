...
var OAuthAdapter = function (pConsumerSecret, pConsumerKey, pSignatureMethod) {
    ...
    var consumerSecret = pConsumerSecret, consumerKey = pConsumerKey, signatureMethod = pSignatureMethod, pin = null, requestToken = null, requestTokenSecret = null, accessToken = null, accessTokenSecret = null, serviceExtras = null, accessor = {
    ...
    this.sendTwitterImage = function (options) {
        ...
        var header = OAuth.getAuthorizationHeader(pUrl, message.parameters);
        client.setRequestHeader("Authorization", header);
        //	client.setRequestHeader("Content-Type", "multipart/form-data");

        client.send(options.parameters);
        ...
        token.serviceExtras && (serviceExtras = token.serviceExtras);
        ...
        delete accessTokenStore[pService], Ti.App.Properties.setString("Social.js-AccessToken-" + pService, null), serviceExtras = null, accessToken = null, accessTokenSecret = null;
    ...
    var createMessage = function (pUrl, method) {
        ...
        return message.parameters.push(["oauth_consumer_key", consumerKey]), message.parameters.push(["oauth_callback", "http://liferay.com/security"]), message.parameters.push(["oauth_signature_method", signatureMethod]), message;
    };
    ...
    this.showLoadingUI = function () {
        ...
        Ti.Android && ( offset = "10dp"), webView = Ti.UI.createWebView({
            ...
            bottom: offset,
            left: offset
        }), webView.addEventListener("beforeload", showLoading), webView.addEventListener("load", authorizeUICallback), view.add(webView), webView.show();
        ...
        var parameterMap = OAuth.getParameterMap(message.parameters), client = Ti.Network.createHTTPClient({
            onload: function () {
                ...
                accessToken = responseParams.oauth_token, accessTokenSecret = responseParams.oauth_token_secret, serviceExtras = responseParams, callback({
                    success: !0
                });
            },
            onerror: function () {
        ...
        var fields = ['first-name', 'last-name', 'formatted-name', 'headline', 'location', 'industry', 'current-share', 'num-connections',
            'summary', 'specialties', 'positions', 'picture-urls::(original)'];

        var pUrl = "http://api.linkedin.com/v1/people/~:(" + fields.join(',') + ")?format=json";
        var pSuccessMessage = options.onSuccess;
        var pErrorMessage = options.onError;

        accessor.tokenSecret = accessTokenSecret;
        var message = createMessage(pUrl, 'GET');
        ...
        client.send();
    ...
    }, this.getProfileXING = function (options) {
            Ti.API.info("token: " + accessToken + " secret: " + accessTokenSecret);

            var pUrl = "https://api.xing.com/v1/users/me";
            var pSuccessMessage = options.onSuccess;
            var pErrorMessage = options.onError;
            ...
            // strip out the oauth_callback for XING API requests
            message.parameters = message.parameters.filter(function(el) {
                return (el[0] != 'oauth_callback');
            });
...
//			message.parameters.push(pParameters[p]);
        ...
        this.getProfileTwitter = function (options) {

            var pUrl = "https://api.twitter.com/1.1/users/show.json";
            var pSuccessMessage = options.onSuccess;
            var pErrorMessage = options.onError;
            ...
            message.parameters.push(["oauth_token", accessToken]);
            message.parameters.push(["oauth_version", "1.0"]);
            message.parameters.push(["user_id", serviceExtras.user_id]);
            ...
            client.open("GET", pUrl + '?user_id=' + encodeURIComponent(serviceExtras.user_id));
            header = OAuth.getAuthorizationHeader("", message.parameters);
        ...
        this.getServiceExtras = function () {
...
exports.create = function (settings) {
    ...
    return urls == null ? (alert("The Social.js module does not support " + site + " yet!"), null) : {
        ...
        getProfileXING: function (options) {
            this.authorize(function () {
                adapter.getProfileXING({
                    site: 'xing',
                    url: urls.update,
                    parameters: options.message,
                    title: "XING",
        ...
        getProfileTwitter: function (options) {
            this.authorize(function () {

                adapter.getProfileTwitter({
                    parameters: options.message,
                    timeout: options.timeout,
                    title: "Twitter",
                    onSuccess: options.success,
