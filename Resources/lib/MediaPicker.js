...
exports.MediaPicker.prototype.show = function(cb, max, type, message) {
    ...
    var done = new Element('Button', {title: L('OK'), bottom: 0, right: 0, zIndex: 99}).addClick(doneFn).element;
    if (_env.iOS) win_assetGroups.setRightNavButton(done);
  //  else win_assetGroups.add(done);

    var cancel = new Element('Button', {title: L('CANCEL'), bottom: 0,left: 0, zIndex: 99}).addClick(function() {
        _nav.closeAll();
    }).element;
    ...
    var gotAssetGroups = function(e) {
        ...
        new Element('TableView', {
            ...
            data:data
                ...
                assets = null;
                win_assets = new Element('Window', {title: e.rowData.caption, backgroundColor: 'white'}).element;
    ...
    function createIV(row, i) {
        ...
        iv.addEventListener(_env.iOS?'singletap':'click', function(e) {
            ...
            if (selectedItems.length == 0) {
                ...
                done.setTitle(L('OK') + ' ('+selectedItems.length+')');
            }
        });
    ...
    MediaPickerModule.getAssetGroups({type: type||'photos', success: gotAssetGroups, error: function() {
        alert(L('PLEASE_ALLOW_PHOTO_ACCESS'));
    }});
